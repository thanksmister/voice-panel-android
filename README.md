...
Currently the application has several limitations.  First, the Snips Android SDK does not work as a satelite.  The Android SDK also does not support custom wake-words.  You must always use "Hey, Snips" with Voice Panel until the Snips team updates the SDK.  Optionally, you can try the face detection wake-word feature which starts listening when it sees a face in the camera. 
